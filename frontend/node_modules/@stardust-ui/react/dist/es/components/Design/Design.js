import * as customPropTypes from '@stardust-ui/react-proptypes'; // @ts-ignore

import { ThemeContext } from 'react-fela';
import * as React from 'react';
import * as PropTypes from 'prop-types';

/**
 * The Design component provides a theme safe subset of CSS for designing layouts.
 */
function Design(_ref) {
  var config = _ref.config,
      children = _ref.children;
  var theme = React.useContext(ThemeContext);
  var getConfig = React.useCallback(function () {
    return config;
  }, [config]); // Heads Up! Keep in sync with getClasses.ts

  var styleParam = {
    theme: {
      direction: theme.rtl ? 'rtl' : 'ltr'
    }
  };
  var className = theme.renderer.renderRule(getConfig, styleParam);
  return children({
    className: className
  });
}

Design.displayName = 'Design';
Design.propTypes = {
  children: PropTypes.func.isRequired,
  config: customPropTypes.design.isRequired
};
export default Design;